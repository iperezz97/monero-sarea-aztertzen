 ...
 * *  - 4. haria: Transakzioak jaso eta nodoekin erlazionatzen dituen haria (recv 2002, mapan marraztu, transakzio kopuruaren estimazioa: 500 byte transakzio bakoitzeko)

 * Konpilatu: gcc nagusia.c bzb.c eskatu1001.c konprob1003.c kokatu_mapan.c jaso2002.c -lpthread -o nagusia
 * Exekutatu: ./nagusia <IP> <PORT> <denb12>
...
// Bukatzeko seinalea (SIGINT) harrapatu eta lortutako nodoen informazioa idatzi 'logbzb' fitxategian //
void intHandler() {
...
// Bukatzeko seinalea (SIGTERM) harrapatu eta lortutako nodoen informazioa idatzi 'logbzb' fitxategian //
void termHandler() {
	printf("q\n"); // python programa ongi bukatzeko
	bzb_inprimatu();
	exit(1);
...
*/
	...
	// transakzioak jaso eskaerak bukatutakoan
...
/* Denbora muga zehaztuta saiatu konexioa ezartzen nodoarekin (1001 eta 1003 komandoak bidaltzeko: 1,2,4 hariek)
