...
//int s2; // socket deskriptorea
struct timeval read_timeout4;

// Hari bat sortu atzigarri dagoen nodo bakoitzeko eta horietatik transakzioak (2007 mezua) jaso
void * jaso_transakzioak(void *args) {
    // log fitxategia sortu
    FILE *log;
    log = fopen("log2002", "w");
    if(log == NULL) {
        printf("Errorea log2002 fitxategia sortzean: %s\n", strerror(errno));
        fflush(stdout);
        pthread_exit(NULL);
    }
    int nodokop = kontatu_atzigarri(root);//
    fprintf(log,"Transakzioak jasotzen...\nAtzigarri dauden nodo kopurua: %d\n", nodokop);
    fflush(log);
    pthread_t trhari[20];
	int i=0;
	int j=0;
...
//	for(i=0; i<nodokop; i++) {
	for(i=0; i<20; i++) {
    	...
    	{
            fprintf (log, "Ezin izan da thread4[%d] haria sortu. Errore zenbakia: %d   Errore mezua: %s \n" , i, errno , strerror(errno));
            fflush(log);
    	}
		fprintf(log, "thread4[%d] haria sortuta\n", i);
		fflush(log);
	}
...
//      ////if(un->egoe < 2 && un->tkop < 0) { // hasieratu haria
//      if(un->egoe < 2) { // hasieratu haria nodo atzigarriarekin
    	pthread_mutex_lock(&(un->lock));		// blokeatu nodoa
        if(un->egoe < 2 && un->lon != 0 && un->lat != 0) { // hasieratu haria nodo atzigarriarekin eta mapan kokatuta
    		ego = un->egoe;
    		un->egoe = 4;    // erreserbatu jaso2002 prozesurako
    		pthread_mutex_unlock(&(un->lock));         // askatu nodoa (egoera horretan ezin atzitu)
    		...
    		pthread_mutex_lock(&(un->lock));		// blokeatu nodoa egoera aldatzeko
    		un->egoe = ego; // hasierako egoerara itzuli beste batzuk prozesua jarraitzeko
...
// Datagram to represent the packet
	...
	int kod = connect_with_timeout(s2, (struct sockaddr *)&dest, sizeof(struct sockaddr), 1000);
...
//		;
    ...
    {
        printf("Errorea goiburukoa bidaltzean. Errore zenbakia: %d  Errore mezua: %s \n" , errno , strerror(errno));
        fflush(stdout);
    	close(s2);
        return (void *) 6;
    }
    ...
    fprintf(log, "1001 eskaeraren goiburukoa bidalita\t\t Pakete tamaina: %d (10), %02x (16).\n", sizesend, sizesend);
    fflush(log);
    ...
    {
        fprintf (log, "Errorea 1001 mezuaren datuak bidaltzean. Errore zenbakia: %d   Error mezua: %s \n" , errno , strerror(errno));
        fflush(log);
	    close(s2);
        return (void *) 7;
    }

    fprintf(log, "1001 eskaeraren datuak bidalita \t\t Pakete tamaina: %d (10), %02x (16).\n", sizesend, sizesend);
    fflush(log);
	...
	// jaso 2002 mezua eta inkrementatu tkop kontagailua -> eragiketa mapari bidaliz (printf("t <lon> <lat> <IP> <tkop>\n");)
	while(1) {
...
//			fflush(log);
			if(b0 == 8) { // header-a bukatzeko 25 byte faltan
                ...
                fprintf(log,"%ld mezua jasota %s:%d-tik! Data tam: %ld \n", comn, target, un->port, tamn);
                fflush(log);
                if(comn == 2002) {
				    ttam=(tamn/500);
                    un->tkop+=ttam; // suposatuz transakzio bakoitzak 500 byte (segurtasunarekin tamaina handiagoa...)
                    fprintf(log,"2002 mezua jasota %s:%d-tik! tkop: %d\n", target, un->port, un->tkop);
                    fflush(log);
                    printf("t %f %f %s %d\n", un->lon, un->lat, target, ttam); // momentuan jaso duena inprimatu
                    fflush(stdout);
                }
                ...
                while(b1 > 0) {
                    ...
                    if (b1 <= 0){
                        fprintf(log, "Mezuaren datuak jasotzean errorea (recvfrom, tamaina %d): %s \n", b1, strerror(errno));
                        fflush(log);
                        close(s2);
                        free(recbuf1);
        				fclose(em);
				...
				if(comn == 2002) {
					...
					fprintf(log,"2002 mezua jasota %s:%d-tik! tkop: %d\n", target, un->port, un->tkop);
					fflush(log);
					printf("t %f %f %s %d\n", un->lon, un->lat, target, ttam); // momentuan jaso duena inprimatu (ez totala)
					fflush(stdout);
				}
				// header-a fitxategian idatzi
...
//				fflush(log);
			...
			fprintf(log, "Ezin mezua jaso, tam: %d  err: %s\n", b0, strerror(errno));
			fflush(log);
			fclose(em);
			unlink(fileizena); // XX.XXX.XX.XXX_2002 ezabatu
			close(s2);
