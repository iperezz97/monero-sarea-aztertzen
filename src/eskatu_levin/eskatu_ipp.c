		...
		if(i==2) {
			...
			memcpy(&psh.tcp , tcph , sizeof (struct tcphdr));
				...
				printf ("Error sending 1001 packet. Error number: %d . Error message: %s \n" , errno , strerror(errno));
                        ...
                        fwrite(datagram+sizeof(struct ip) + sizeof(struct tcphdr), sizeof(char), 226+33, fp);
                     	//fputs("\n", fp);
		        fclose(fp);
...
//		char * soip = inet_addr(&iph->saddr);
//		char * doip = inet_addr(&iph->daddr);


		if(tcph->syn == 1 && tcph->ack == 1 && source.sin_addr.s_addr == dest_ip.s_addr )
		{
			printf("ACK received. Port %d open %d %d \n", ntohs(tcph->source), iph->saddr, iph->daddr);
