/*
 * $ gcc eskatu_ipp.c -lpthread -o eskatu_ipp
 * $ sudo ./eskatu_ipp XXX.XXX.XXX.XXX PPPPP
 */
...
int main(int argc, char *argv[])
{
        ...
        // IP header
        struct iphdr *iph1 = (struct iphdr *) datagram1;
        struct iphdr *iph0 = (struct iphdr *) datagram0;
        struct iphdr *iph = (struct iphdr *) datagram;

        // TCP header
        struct tcphdr *tcph1 = (struct tcphdr *) (datagram1 + sizeof (struct ip));
        ...
        // Payload
        struct data *dat = (struct data *) (datagram1 + sizeof(struct ip) + sizeof(struct tcphdr) );
        ...
        tcph->check = 0; // if you set a checksum to zero, your kernel's IP stack should fill in the correct checksum during transmision
        tcph->urg_ptr = 0;
	...
	for(i = 1 ; i < 3 ; i++ )
	{
		...
		if(i==2) {
			...
			memcpy(&psh.tcp , tcph , sizeof (struct tcphdr));
                        ...
                        int sizesend = sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(struct levhdr);
                        // Send the Levin handshake request packet
                        if ( sendto (s, datagram, sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(struct levhdr) , 0 , (struct sockaddr *) &dest, sizeof (dest)) < 0)
                        {
				printf ("Error sending 1001 packet. Error number: %d . Error message: %s \n" , errno , strerror(errno));
                        ...
                        printf("\n1001 request header sent to: %s\t Packet size: %d = %x.\n", m, sizesend, sizesend);
                        int itr;
                        ...
                        fwrite(datagram+sizeof(struct ip) + sizeof(struct tcphdr), sizeof(char), 33, fp);
                     	//fputs("\n", fp);
		        fclose(fp);
			...
			// bidali 1001 mezuaren gorputza
                        memcpy(iph1 , iph , sizeof (struct iphdr));
                        memcpy(tcph1 , tcph , sizeof (struct tcphdr));

			memcpy(&psh.tcp , tcph1 , sizeof (struct tcphdr));

                        tcph1->check = csum( (unsigned short*) &psh , sizeof (struct pseudo_header));
                        sizesend = sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(struct data);
                        // Send the Levin handshake request body
                        if ( sendto (s, datagram1,  sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(struct data) , 0 , (struct sockaddr *) &dest, sizeof (dest)) < 0)
                        {
                                printf ("Error sending 1001 packet. Error number: %d . Error message: %s \n" , errno , strerror(errno));
                        ...
                        printf("\n1001 request body sent to: %s\t Packet size: %d = %x.\n", m, sizesend, sizesend);
                        //int itr;
                        //  gorde fitxategian bidalitako levin eskaera
                        ...
                        fwrite(datagram1+sizeof(struct ip) + sizeof(struct tcphdr), sizeof(char), 226, fp2);
                        //fputs("\n", fp);
                        fclose(fp2);
			sleep(3);
...
*/
	if(iph->protocol == 6) // 6 == TCP
	{
		...
		struct levhdr *levh=(struct levhdr*)(buffer + iphdrlen + sizeof(struct tcphdr*));
		...
		{
			printf("ACK received. Port %d open %s %s \n", ntohs(tcph->source), inet_ntoa(source.sin_addr), inet_ntoa(dest.sin_addr));
			//printf("My host: %s and other's: %s \n", soip, doip);
			fflush(stdout);
		}
